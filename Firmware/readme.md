...
User configuration is normally saved during shutdown, but this step is skipped when a firmware installation has just completed. The `installFlag` prevents the newly installed firmware from being overwritten by the existing persistent configuration during the shutdown that immediately follows installation:
...
The `installFlag` provides a final safeguard during this transition. Because the outgoing firmware has been replaced, `powerStop` skips its normal user-state backup when an update has just been installed. The flag itself does not survive the power cycle; the app can instead offer to restore the user's previous settings after the new firmware boots and reconnects.
...
The actual NRZ waveform is generated by `doGRB`. The routine accepts a 24-bit color value `p0` and an output-pin mask `p1`, adjusts the color code for the current brightness level, and then shifts the resulting 24-bit value out to the LEDs. SK6812 devices require the color bytes in green-red-blue (GRB) order.


```spin
:clock        mov       s0,     cnt                 'mark time for set timer
...
:bits         mov       s5,     s0                  'copy set timer to reset timer
              waitcnt   s0,     tTOT                'wait for set deadline
              or        outa,   p1                  'set output pin high
              add       s5,     t0H                 'set reset deadline
              test      p0,     s3              wc  'set C flag to masked bit state
if_c          add       s5,     t0H                 'update reset deadline if bit high
              waitcnt   s5,     #0                  'wait for reset deadline
              andn      outa,   p1                 ''reset output pin low
              shr       s3,     #1                  'shift bit mask right one bit
              djnz      s1,     #:bits              'decrement bit counter and jump to :bits if not zero
              djnz      s2,     #:pixel             'decrement pixel counter and jump to :pixel if not zero
              waitcnt   s0,     tRST                'wait for cycle deadline and prepare reset interval
              waitcnt   s0,     #0                  'wait for reset deadline
```

The timing-critical portion establishes an absolute deadline for every bit rather than delaying relative to the previous instruction. This timing structure is important because PASM instructions normally execute in four system-clock cycles. By establishing an absolute deadline for each bit, the routine can perform the required per-bit operations without allowing small variations in instruction-path length to accumulate from one bit to the next. Each successive `waitcnt` targets the next scheduled transmission boundary. After the complete pixel stream, the final `waitcnt` generates a reset interval which causes the data line to remain LOW for the required period before the next transmission can begin.
