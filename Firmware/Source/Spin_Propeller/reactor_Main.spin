...
'system objects
  ...
  sns   : "reactor_Sensors"     'sensor control
  dmp   : "reactor_DMP"         'digital motion processor
  led   : "reactor_LED"         'led driver
...
'initialize Reactor subsystems
  ...
  dmpStart                      'start motion processing control cog
...
'launch sensor control in new cog
  RDYsensors:=sns.start(LockID2,LockID3,LockID5,@imuData,@snsErr,@newImuData,@newPowerData,@powerLevel,@abHold)
...
'launch sensor fusion in new cog
  RDYdmp:=dmp.start(LockID3,LockID4,@newImuData,@newFusionData,@imuData,@fusionData,@fusionCalibration)
  repeat until RDYdmp
...
'launch led control in new cog
  RDYled:=led.start(LockID1,@ledData)
  repeat until RDYled
...
firmwareRev   byte "VERSION 00.03.02",0
updateRev     byte "00.03.02",0
'color code banks
