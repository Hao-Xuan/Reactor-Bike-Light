...
To accommodate these competing requirements, the system was divided into functional subsystems organized around four power domains. This architecture simplified development and debugging while allowing the sensing, communications, control, and lighting hardware to evolve independently as the design matured. This architecture also supports Reactor's low-power operating modes by allowing portions of the system to remain active only when required.
...
**Figure 2** - Power Source circuitry showing the removable battery interface and input protection stage
...
**Figure 3** - Always-On Domain showing standby power, touch sensing, and wakeup logic
...
**Figure 4** - High-Power Domain power distribution and startup sequencing
#### Power Distribution
...
The lighting system consists of two independent chains of addressable RGB LEDs. Each array contains eight LEDs and is controlled independently by firmware through dedicated single-wire serial data connections `LED_A` and `LED_B`. Figure 5 shows the schematic snippet for both LED arrays.
...
**Figure 5** - Dual independently controlled addressable LED arrays
...
**Figure 6** - Control Domain 3.3 V power distribution and startup sequencing

The 3.3 V regulator is enabled by the ``+3V3_EN`` signal, which is driven by the power-good (PG) output of the 4 V regulator. During 4 V startup, the PG output is held low, keeping the 3.3 V regulator disabled. Once the 4 V output has stabilized, PG releases to its open-drain state and an external pull-up resistor drives ``+3V3_EN`` high, enabling the 3.3 V regulator.
...
**Figure 8** - Inertial measurement subsystem
...
**Figure 9** - Bluetooth Low Energy communications subsystem
...
The module communicates with the microcontroller through a UART interface. A dedicated`BLE_RST` signal allows firmware to place the module in a known state during startup and recover from communication faults if necessary. Two configurable GPIO outputs are connected to the microcontroller as`BLE_P1_6` and`BLE_P1_7`, providing simple hardware status signals that reduce firmware polling overhead. The remaining module I/O pins are unused and tied to their recommended default states in accordance with the manufacturer's reference design.
...
**Figure 10** - Battery monitoring analog front-end
...
**Figure 11** - Persistent memory subsystem
...
**Figure 13** - Top layer showing component placement, signal routing, and battery power distribution
...
**Figure 14** - Layer 2 dedicated ground plane
...
**Figure 15** - Layer 3 regulated power distribution
...
**Figure 16** - Bottom layer dedicated ground plane
...
The Reactor electronics were validated progressively throughout development, beginning with the power-input circuitry and individual regulated domains and culminating in complete system bring-up and thermal testing. The development PCB provided access to programming interfaces and test points that were intentionally omitted from the sealed production hardware.
...
2. **4 V rail reaches regulation** — The 4.0 V rail rises to its regulated output, after which its open-drain `PG` output is released.

3. **`+3V3_EN` asserted** — The now high-impedance `PG` output allows `+3V3_EN` to be pulled high, enabling the 3.3 V regulator.
...
5. **`POWER_EN` asserted** — Firmware asserts `POWER_EN` low, latching the system in the ON state. The touch sensors can then be released without interrupting power.
...
The measured sequence occurs on multiple time scales. The initial transitions between the power-control signals and supply rails occur within tens to hundreds of microseconds, while the final transition to the latched ON state occurs on a much longer time scale. From the initial wakeup event to assertion of `POWER_EN`, the complete startup sequence takes approximately 1 second.
...
Thermal performance was evaluated using a controlled static bench test of three Reactor units. Each unit was operated individually in Head (H), Ground (G), and Tail (T) modes under three cooling conditions: ambient (A) cooling by natural convection, low-fan (L) cooling using a low level of forced airflow, and high-fan (F) cooling using a higher level of forced airflow. The tests were performed indoors at an ambient temperature of approximately 27 °C. The progressively higher fan settings were used to examine how additional airflow affected the thermal behavior of the system.

Figure 19 shows the resulting temperature measurements. The three Reactor units exhibit consistent thermal behavior within each operating mode. Head mode produces the highest temperatures, while Ground and Tail modes operate substantially cooler.

<img width="1029" height="1120" alt="Temp (C) vs Time (min)" src="https://github.com/user-attachments/assets/a76ac29d-b0e4-46af-9e02-3e1af9e5bcac" />

**Figure 19** - Temperature versus time for three Reactor units tested in Head, Ground, and Tail modes under ambient, low-fan, and high-fan cooling conditions.

The most significant result is the repeatable temperature regulation observed in Head mode. Under ambient cooling, all three units converge to a narrow operating range of approximately 59–60 °C. Once this temperature range is reached, the traces exhibit small, regular changes in temperature as the firmware's thermal regulation adjusts LED output in response to the measured temperature. The similar behavior observed across all three units demonstrates that the thermal regulation mechanism operates consistently between units rather than depending on the characteristics of an individual board.

Forced airflow substantially lowers the operating temperature of the Reactor. Low-fan operation reduces the Head-mode temperature to approximately the low-to-mid 50 °C range, while the high-fan condition produces a further reduction in some cases. The difference between the two fan settings is less pronounced than the difference between forced airflow and ambient cooling, indicating that additional airflow provides diminishing thermal benefit once sufficient cooling is available.

Ground and Tail modes remain substantially cooler than Head mode under all three cooling conditions. This is consistent with their lower LED power levels and further demonstrates that the thermal-control system is primarily exercised under the highest-load Head-mode condition.

These bench tests provide validation that Reactor's thermal behavior is repeatable across multiple units and that the firmware's temperature regulation maintains a controlled operating range under the highest-power lighting condition. The controlled cooling experiments also demonstrate that the system responds predictably to changes in available airflow. Static bench testing does not, however, reproduce the complete thermal environment of a moving bicycle; real-world cooling performance will therefore be evaluated separately through road testing.
